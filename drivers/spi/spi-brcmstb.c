...
static int brcmstb_spi_probe(struct platform_device *pdev)
{
	...
	priv->mspi_regs = devm_ioremap_resource(dev,
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "hif_mspi"));
	if (IS_ERR(priv->mspi_regs)) {
		spi_master_put(master);
		return PTR_ERR(priv->mspi_regs);
	}

	priv->bspi_regs = devm_ioremap_resource(dev,
		platform_get_resource_byname(pdev, IORESOURCE_MEM, "bspi"));
	if (IS_ERR(priv->bspi_regs)) {
		spi_master_put(master);
		return PTR_ERR(priv->mspi_regs);
	}
