...
#define MSPI_INTR_CPU_STATUS		0x0
#define MSPI_INTR_CPU_SET		0x4
#define MSPI_INTR_CPU_CLEAR		0x8
#define MSPI_INTR_CPU_MASK_STATUS	0xc
#define MSPI_INTR_CPU_MASK_SET		0x10
#define MSPI_INTR_CPU_MASK_CLEAR	0x14
#define MSPI_INTR_DONE_MASK		0x00000020
...
static irqreturn_t brcmstb_spi_interrupt(int irq, void *dev_id)
{
	...
	if (priv->mspi_regs->mspi_status & 0x1) {
		...
		val = __raw_readl(priv->intr_regs + MSPI_INTR_CPU_CLEAR);
		__raw_writel(val | MSPI_INTR_DONE_MASK,
				priv->intr_regs + MSPI_INTR_CPU_CLEAR);
		__raw_readl(priv->intr_regs + MSPI_INTR_CPU_CLEAR);
...
static int brcmstb_spi_probe(struct platform_device *pdev)
{
	...
	priv->intr_regs = devm_ioremap_resource(dev,
			platform_get_resource(pdev, IORESOURCE_MEM, 1));
	if (IS_ERR(priv->intr_regs)) {
		spi_master_put(master);
		return PTR_ERR(priv->intr_regs);
	}
	...
	__raw_writel(0xffffffff, priv->intr_regs + MSPI_INTR_CPU_MASK_SET);
	__raw_readl(priv->intr_regs + MSPI_INTR_CPU_MASK_SET);

	__raw_writel(0xffffffff, priv->intr_regs + MSPI_INTR_CPU_CLEAR);
	__raw_readl(priv->intr_regs + MSPI_INTR_CPU_CLEAR);

	val = __raw_readl(priv->intr_regs + MSPI_INTR_CPU_MASK_CLEAR);
	__raw_writel(val | MSPI_INTR_DONE_MASK,
			priv->intr_regs + MSPI_INTR_CPU_MASK_CLEAR);
	__raw_readl(priv->intr_regs + MSPI_INTR_CPU_MASK_CLEAR);
