...
int __init brcmstb_bmem_dt_fixup(void)
{
	...
	const char *p;
	int current_len;
	u64 rsv_base, rsv_size, base, size;
	int i;
	...
	rsv_base = CONFIG_BRCMSTB_BMEM_DT_FIXUP_NOMAP_ADDR;
	rsv_size = CONFIG_BRCMSTB_BMEM_DT_FIXUP_NOMAP_SIZE;

	for (i = 0; i < fdt_num_mem_rsv(fdt); i++) {
		if (fdt_get_mem_rsv(fdt, i, &base, &size) < 0)
			continue;
		if (((rsv_base < (base + size)) && (base < (rsv_base + rsv_size))))
			fdt_del_mem_rsv(fdt, i);
	}

	fdt_add_mem_rsv(fdt, rsv_base, rsv_size);
